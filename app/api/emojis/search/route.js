...
export async function GET(request) {
  ...
  const filteredEmojis = emojis.filter((emoji) => {
    return (
      emoji.unicodeName.toLowerCase().includes(queryName.toLowerCase()) ||
      emoji.group.toLowerCase().includes(queryName.toLowerCase()) ||
      emoji.subGroup.toLowerCase().includes(queryName.toLowerCase())
    );
  });
